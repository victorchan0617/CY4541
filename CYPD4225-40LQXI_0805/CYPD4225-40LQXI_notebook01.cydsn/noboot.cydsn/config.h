...
/*******************************************************************************
 * Fast Role Swap detection workaround using HPD block
 *
 ...
 *
 * Note: If this feature is enabled then the fast role swap is also enabled
 ******************************************************************************/
...
#if (!FRS_WORKAROUND_ENABLE)
#define APP_VBUS_SNK_FET_ON_P1()                    VBUS_C_CTRL_P1_Write(0)
#else
#define APP_VBUS_SNK_FET_ON_P1()                                        \
{                                                                       \
    SRSSLT->tst_ddft_ctrl &=~(TST_DDFT_CTRL_DFT_SEL0_MASK);             \
    SRSSLT->tst_ddft_ctrl |= (15 << TST_DDFT_CTRL_DFT_SEL0_POS);        \
}
#endif /* FRS_WORKAROUND_ENABLE */
...
{                                                                       \
    SRSSLT->tst_ddft_ctrl &=~(TST_DDFT_CTRL_DFT_SEL0_MASK);             \
    SRSSLT->tst_ddft_ctrl |= (12 << TST_DDFT_CTRL_DFT_SEL0_POS);        \
}
#endif /* FRS_WORKAROUND_ENABLE */
/*******************************************************************************
 ...
 *
 * The blinking LED is disabled to allow SWD debug, since it uses the SWD_IO
 * GPIO. It is also recommended to disable LED blinking for production designs
 * to save power.
 */
